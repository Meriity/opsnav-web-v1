...
export default function Stage5({
        ...
        if (currentModule === "commercial") {
            ...
            setNoteForClient(loadedClientComment);
             ...
             noteGroups.forEach((group) => {
                ...
                const noteParts = noteString.split(" - ").filter(p => p.trim() !== "");
                // part[0] is system note, rest is client comment
                const clientComment = noteParts.length > 1 ? noteParts.slice(1).join(" - ").trim() : "";
  ...
  const generateSystemNote = (groupId = "main") => {
      ...
      const fieldsToCheck = fields.filter(f => noteGroup.fieldsForNote.includes(f.name));
      ...
      return missing.length === 0 ? "All tasks completed" : `Pending: ${missing.join(", ")}`;
  };
  ...
  const isChanged = () => {
       const formChanged = JSON.stringify(formState) !== JSON.stringify(originalData.current);
       // Check commercial note separate state
       if (currentModule === "commercial") {
           return formChanged || (noteForClient !== (originalData.current.noteForClient || ""));
       }
       return formChanged;
  };
  ...
  const handleSave = async () => {
    ...
    if (currentModule === "commercial") {
       ...
       const cleanPayload = {};
       fields.forEach(f => {
           if (payload[f.name] !== undefined) cleanPayload[f.name] = payload[f.name];
       });
               ...
               <div className="flex flex-wrap gap-x-8 gap-y-2">
                  {["Yes", "No", "Processing", "N/R"].map((v) => (
                    <label key={v} className="flex gap-2">
                       <input
                         type="radio"
                         checked={normalizeValue(formState[f.name]) === normalizeValue(v)}
